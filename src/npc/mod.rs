...
impl GameEntity<([&mut Player; 2], &NPCList, &mut Stage, &mut BulletManager, &mut Flash, &mut BossNPC)> for NPC {
    fn tick(
        ...
        match self.npc_type {
            ...
            // 332 => self.tick_n332_ballos_shockwave(state),
            // 333 => self.tick_n333_ballos_lightning(state),
            ...
            // 338 => self.tick_n338_green_devil(state),
            // 339 => self.tick_n339_green_devil_generator(state),
            340 => self.tick_n340_ballos(state, players, npc_list, flash),
            // 341 => self.tick_n341_ballos_1_head(state),
            // 342 => self.tick_n342_ballos_1_eye(state),
