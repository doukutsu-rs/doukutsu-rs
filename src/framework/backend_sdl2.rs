                ...
                match event {
                    ...
                    Event::Window { win_event, .. } => match win_event {
                        ...
                        WindowEvent::SizeChanged(width, height) => {
                            ctx.real_screen_size = (width.cast_unsigned(), height.cast_unsigned());
                            ctx.screen_size = get_scaled_size(width.max(1).cast_unsigned(), height.max(1).cast_unsigned());
                    ...
                    Event::FingerDown { timestamp, touch_id, finger_id, x, y, dx, dy, pressure } => {
                        let touch_id = touch_id.cast_unsigned();
