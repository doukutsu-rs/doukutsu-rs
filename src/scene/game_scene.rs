...
use crate::components::lights_2d::Lights2D;
...
use crate::framework::graphics::draw_rect;
...
impl Scene for GameScene {
    ...
    fn draw(&self, state: &mut SharedGameState, ctx: &mut Context) -> GameResult {
        ...
        if state.settings.shader_effects && self.lighting_mode == LightingMode::BackgroundOnly {
            self.lights_2d.draw_light_map(state, ctx, self)?;
        }
