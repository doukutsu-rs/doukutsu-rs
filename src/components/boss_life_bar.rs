...
impl GameEntity<(&NPCList, &BossNPC, &NPCAccessToken)> for BossLifeBar {
    fn tick(&mut self, _state: &mut SharedGameState, (npc_list, boss, token): (&NPCList, &BossNPC, &NPCAccessToken)) -> GameResult<()> {
        match self.target {
